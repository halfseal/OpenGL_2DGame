...
#include "ut.h"	// slee's OpenGL utility
...
bool init_text() {
    ...
    if (!result) {
        printf("%s(): Failed to initialize stb_truetype.\n", __func__);
        return false;
    }
    create_font_textures();
    free(font_buffer);

    program_text = create_program(vert_text_path, frag_text_path);
    if (!program_text)
        return false;
    ...
    VAO = create_va(vertex_buffer);
    if (!VAO) {
        printf("%s(): VAO==nullptr\n", __func__);
        return false;
    }
...
float CalculateDPIScale(float reference_DPI = 96.0f, float adjustment_factor = 1.0f) {
    HDC screen = GetDC(0);
    float dpiX = static_cast<float>(GetDeviceCaps(screen, LOGPIXELSX));
    ReleaseDC(0, screen);
    float dpi_scale = (reference_DPI / dpiX) * adjustment_factor;
    return dpi_scale;
}

void render_text(ivec2 &window_size, float tmp, std::string text, GLint _x, GLint _y, GLfloat scale, vec4 color) {
    ...
    mat4 text_offset = mat4(
        1 / (window_size.x / 2.0f), 0.0f, 0.0f, -1.0f, // view space conversion
    ...
    for (auto c = text.begin(); c != text.end(); c++) {
        advance += scale * (float)stbtt_char_list[*c].advance;
    }
    ...
    for (c = text.begin(); c != text.end(); c++) {
        ...
        mat4 text_translate =
            mat4(1.0f, 0.0f, 0.0f, x + scale * float(ch.bearing.x) - advance / 2.0f,
        ...
        text_matrix = trans_mx * text_offset * text_translate * text_size * text_matrix;
